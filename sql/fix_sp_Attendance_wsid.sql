-- Add WorkScheduleId tracking to sp_Attendance
...
BEGIN
    ...
    BEGIN
        BEGIN TRY
            SELECT
                ...
                a.WorkScheduleId, ws.Name AS WorkScheduleName
            FROM Attendances a
            INNER JOIN Employees e ON e.Id = a.EmployeeId
            LEFT JOIN WorkSchedules ws ON ws.Id = a.WorkScheduleId
    ...
    BEGIN
        BEGIN TRY
            ...
            -- Auto-resolve schedule if not provided
            IF @WorkScheduleId IS NULL
            BEGIN
                SELECT TOP 1 @WorkScheduleId = WorkScheduleId
                FROM EmployeeSchedules
                WHERE EmployeeId = @EmployeeId AND EndDate IS NULL AND IsDeleted = 0
                ORDER BY EffectiveDate DESC;

                IF @WorkScheduleId IS NULL
                    SELECT TOP 1 @WorkScheduleId = Id FROM WorkSchedules WHERE IsDefault = 1 AND IsDeleted = 0;
            END
            ...
            INSERT INTO Attendances (PayrollPeriodId, EmployeeId, WorkScheduleId, DaysWorked, TotalDays, LateHours, UndertimeHours, OtHours, NightDiffHours, Status, Issue, CreatedAt, CreatedBy)
            VALUES (@PeriodId, @EmployeeId, @WorkScheduleId, @DaysWorked, @TotalDays, @LateHours, @UndertimeHours, @OtHours, @NightDiffHours, @CrStatusInt, @Issue, GETDATE(), @CreatedBy);
    ...
    BEGIN
        BEGIN TRY
            ...
            BEGIN
                ...
                BEGIN
                    -- Resolve schedule for this employee
                    DECLARE @ImpSchedId INT = NULL;
                    SELECT TOP 1 @ImpSchedId = WorkScheduleId
                    FROM EmployeeSchedules WHERE EmployeeId = @ImpEId AND EndDate IS NULL AND IsDeleted = 0
                    ORDER BY EffectiveDate DESC;
                    IF @ImpSchedId IS NULL
                        SELECT TOP 1 @ImpSchedId = Id FROM WorkSchedules WHERE IsDefault = 1 AND IsDeleted = 0;

                    DECLARE @ImpAS INT = CASE WHEN @ImpDW=0 THEN 3 WHEN @ImpDW<@ImpTD THEN 2 ELSE 1 END;
                    INSERT INTO Attendances (PayrollPeriodId,EmployeeId,WorkScheduleId,DaysWorked,TotalDays,LateHours,UndertimeHours,OtHours,NightDiffHours,Status,CreatedAt,CreatedBy)
                    VALUES (@PeriodId,@ImpEId,@ImpSchedId,@ImpDW,@ImpTD,@ImpLH,@ImpUH,@ImpOH,@ImpNH,@ImpAS,GETDATE(),@CreatedBy);
        ...
        BEGIN TRY
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    INSERT INTO Attendances (PayrollPeriodId, EmployeeId, WorkScheduleId, DaysWorked, TotalDays, LateHours, UndertimeHours, OtHours, NightDiffHours, Status, Issue, CreatedAt, CreatedBy)
                    VALUES (@PeriodId, @RpEmpId, @RpSchedId, @RpDaysPresent, @RpTotalDaysCount, @RpTotalLate, @RpTotalUT, @RpTotalOT, @RpTotalND, @RpAttStatus, @RpIssueText, GETDATE(), @CreatedBy);
    ...
    BEGIN
        BEGIN TRY
            ...
            IF @GesSchedId IS NULL
            BEGIN
                SELECT CAST(0 AS INT) AS DayOfWeek, CAST('08:00' AS NVARCHAR(5)) AS ShiftStart,
                    CAST('17:00' AS NVARCHAR(5)) AS ShiftEnd, CAST(0 AS BIT) AS IsRestDay,
                    CAST(15 AS INT) AS GracePeriodMinutes, CAST(0 AS INT) AS OTStartAfterMinutes,
                    CAST(30 AS INT) AS OTMinimumMinutes, CAST('22:00' AS NVARCHAR(5)) AS NightDiffStartTime,
    ...
    BEGIN
        BEGIN TRY
            ...
            SELECT TOP 1
                @RepGrace = GracePeriodMinutes, @RepOTAfter = OTStartAfterMinutes,
                @RepOTMin = OTMinimumMinutes, @RepNDStart = NightDiffStartTime, @RepNDEnd = NightDiffEndTime
            FROM ScheduleRules WHERE IsDeleted = 0;
            ...
            BEGIN
                ...
                FROM WorkSchedules WHERE Id = @RepSchedId AND IsDeleted = 0;
            END
            ...
            DECLARE rep_cursor CURSOR LOCAL FAST_FORWARD FOR
                ...
                FROM AttendanceDetails WHERE AttendanceId = @RepAttId AND IsDeleted = 0;
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    UPDATE AttendanceDetails SET
                        LateHours = @RepLate, UndertimeHours = @RepUT,
                        OtHours = @RepOT, NightDiffHours = @RepND,
                        UpdatedBy = @UpdatedBy, UpdatedAt = GETDATE()
                    WHERE Id = @RepDetId;
                END
            ...
            -- Update WorkScheduleId on the attendance record too
            UPDATE Attendances SET
                WorkScheduleId = @RepSchedId,
